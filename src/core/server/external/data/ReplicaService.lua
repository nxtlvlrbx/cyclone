...
export type Replica = {
	...
	SetValue: (Replica, path: string | { string }, newValue: any?) -> (),
	SetValues: (Replica, path: string | { string }, ...any) -> (),
	ArrayInsert: (Replica, path: string | { string }, newValue: any) -> number,
	ArraySet: (Replica, path: string | { string }, index: number, newValue: any) -> (),
	ArrayRemove: (Replica, path: string | { string }, index: number) -> any,
	Write: (Replica, string, ...any) -> ...any,
	SetParent: (Replica, parent: Replica) -> (),
	ReplicateFor: (Replica, "All" | Player) -> (),
	DestroyFor: (Replica, "All" | Player) -> (),
	ConnectOnServerEvent: (Replica, Player, ...any) -> RBXScriptConnection,
	FireClient: (Replica, Player, ...any) -> (),
	FireAllClients: (Replica, Player, ...any) -> (),
	Destroy: (Replica) -> (),
}
	...
	elseif replication_settings == nil then
		...
		if type(replication_settings) == "table" then -- Must be a player list {player = true, ...} OR an empty table {}
			...
			for player in pairs(replication_settings) do
				if
					not (typeof(player) == "Instance" and player.ClassName == "Player")
					and not (typeof(player) == "table" and player.PlayerObject)
				then
