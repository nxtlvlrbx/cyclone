...
export type Replica = {
    ...
    Parent: Replica,
    Children: {},
    IsActive: (Replica) -> boolean,
    Identify: (Replica) -> string,
    AddCleanupTask: (Replica) -> () | Instance,
    RemoveCleanupTask: (Replica) -> () | Instance,
	ListenToWrite: (Replica, string, (...any) -> ()) -> (RBXScriptConnection),
	ListenToChange: (Replica, path: string | {string}, listener: (...any) -> ()) -> (RBXScriptConnection),
	ListenToNewKey: (Replica, path: string | {string}, listener: (...any) -> ()) -> (RBXScriptConnection),
	ListenToArrayInsert: (Replica, path: string | {string}, listener: (...any) -> ()) -> (RBXScriptConnection),
	ListenToArraySet: (Replica, path: string | {string}, listener: (...any) -> ()) -> (RBXScriptConnection),
	ListenToArrayRemove: (Replica, path: string | {string}, listener: (...any) -> ()) -> (RBXScriptConnection),
	ListenToRaw: (Replica, callback: (...any) -> ()) -> (RBXScriptConnection),
	ListenToChildAdded: (Replica, callback: (...any) -> ()) -> (RBXScriptConnection),
	FindFirstChildOfClass: (Replica, ClassName: string) -> (Replica?),
	ConnectOnClientEvent: (Replica, callback: (...any) -> ()) -> (RBXScriptConnection),
	FireServer: (Replica, ...any) -> (),
}
